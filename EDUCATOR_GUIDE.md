...
| 3 | FTA — medium complexity, gates and cut sets | `FTA_Smart_Infusion_Pump.ipynb` |
...
| 7 | ETA — medium complexity, cyber-induced events | `ETA_Cyber_Induced_Valve_Control_Failure.ipynb` |
| 8 | ETA — complex case study, defense-in-depth | `ETA_Smart_Power_Grid_Substation.ipynb` |
...
| 11 | BN — complex cyber-physical case study | `BN_Chemical_Processing_Plant.ipynb` |
...
| 4 | Complex case-study comparison: chemical plant via FTA, ETA, and BN | `FTA_Chemical plant_casestudy.ipynb`, `ETA_Smart_Power_Grid_Substation.ipynb`, `BN_Chemical_Processing_Plant.ipynb` |
...
3. `ETA_Smart_Power_Grid_Substation` — cyber-induced failure in a smart substation
4. `FTA_Library_Chemical_Plant_.ipynb` — chemical plant cybersecurity-safety failure
5. `BN_Chemical_Processing_Plant.ipynb` — cyber-physical risk in chemical processing
...
**`ETA_Cyber_Induced_Valve_Control_Failure`** — Cyber-induced valve-control failure on an oil pipeline. Good for discussing how an initiating event can be either an accidental or adversarial trigger.

**`ETA_Smart_Power_Grid_Substation.ipynb`** — Six-layer defense-in-depth in a smart power-grid substation. Use this notebook to discuss layered protection, residual risk, and the value of human-in-the-loop barriers.
...
**`BN_Chemical_Processing_Plant.ipynb`** — Eight nodes, ten edges, multiple inference queries. Emphasise the *explaining away* effect when conditioning on a collider, and the practical challenge of populating CPTs from limited data or expert judgement.
